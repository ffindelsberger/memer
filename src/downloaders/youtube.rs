...
async fn download_file(url: &str, filename: &str) -> LoadResult<PathBuf> {
    // Because i am changing the working dir of the Child it does not find the yt-dlp_macos binary so i made the path ot the program also canonical
    ...
    let cmd = f!(r#"-f "b[ext=mp4]" -S "filesize~7M" -o {}"#, filename);
...
mod test {
    ...
    async fn test_download_file_full_video() -> Result<(), String> {
        match download_file("https://www.youtube.com/shorts/B1j3yeHRKbY", "test1").await {
            Ok(file_path) => {
    ...
    async fn test_download_youtube_shorts() -> Result<(), String> {
        match download_file("https://www.youtube.com/watch?v=TK4N5W22Gts", "test2").await {
            Ok(file_path) => {
                assert!(file_path.exists());
                fs::remove_file(file_path).expect("Panicked while deleting File in test");
            }
    ...
    async fn test_download_youtube_share_link() -> Result<(), String> {
        match download_file("https://youtu.be/UT5F9AXjwhg", "test3").await {
            Ok(file_path) => {
                assert!(file_path.exists());
                fs::remove_file(file_path).expect("Panicked while deleting File in test");
            }
